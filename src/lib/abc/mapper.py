...
class MapperProtocol[TInput, TOutput](Protocol):
    @abc.abstractmethod
    def decode(self, message: Any) -> TInput: ...  # noqa: ANN401

    async def validate(self, message: TInput) -> Result[None, Exception]:  # noqa: ARG002
        return Ok(None)

    @abc.abstractmethod
    async def map(self, message: TInput) -> TOutput: ...

    async def process(self, message: Any) -> Result[TOutput, Exception]:  # noqa: ANN401
